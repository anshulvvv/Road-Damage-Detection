...
import json
import argparse
from xml_to_yolo import main as xty
from yolo_to_coco import main as ytc

def xml_to_yolo(xml_path, yolo_path):
    xty(xml_path, yolo_path)

def yolo_to_coco(yolo_files, output_path):
    ytc(yolo_files, output_path)
...
def main(base_dir, val_split=0.1):
    ...
    yolo_to_coco(
        ...
        os.path.join(base_dir, 'train', 'images','annotations_coco.json')
    )
    yolo_to_coco(
        [os.path.join(yolo_labels_val, f) for f in os.listdir(yolo_labels_val)],
        os.path.join(base_dir, 'val','images', 'annotations_coco.json')
    )
...
if __name__ == "__main__":
    print("xty is callable:", callable(xty))
    print("ytc is callable:", callable(ytc))
    args = parse_args()
